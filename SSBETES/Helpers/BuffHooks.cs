...
namespace SSBETES.Helpers
{
    internal sealed class BuffHooks
    {
        ...
        {
            var tHelper = SSBETES.DBZMOD.Code.DefinedTypes.First(x => x.Name.Equals("TransformationHelper"));
        ...
        public static void SetStaticDefaults_Hook(orig_SetStaticDefaults orig, dynamic self)
        {
            ...
            if (DBT_Adjustments.TryGetValue(name, out var adjustments))
            {
                ...
                string tip = (string)SSBETES.DBZMOD.Code.DefinedTypes.First(x => x.Name.Equals(name)).GetMethod("AssembleTransBuffDescription").Invoke(self, null);
        ...
        public static void Update_Hook(orig_Update orig, dynamic self, Player player, ref int buffIndex)
        {
            ...
            if(DBT_Adjustments.TryGetValue(name, out var adjustments))
            {
                ...
                if(name == "SSJRBuff")
                {
                    var type = SSBETES.DBCA.Code.DefinedTypes.First(x => x.Name.Equals("dbzcalamityPlayer"));
